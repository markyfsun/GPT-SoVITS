...
def vc_main(wav_path, text, language, prompt_wav, noise_scale=0.5):
    ...
    phones_tensor = torch.LongTensor(phones).to('cuda')
    quantized_shape_tensor = torch.LongTensor([quantized.shape[-1]]).to('cuda')
    phones_length_tensor = torch.LongTensor([len(phones)]).to('cuda')
    ge = ge.to('cuda')
